...
@app.on_callback_query(
    filters.regex(
        pattern=r"^(SEARCHANSWER|PLAYMODEANSWER|PLAYTYPEANSWER|AUTHANSWER|ANSWERVOMODE|VOTEANSWER|AUTOPLAYANSWER|SYNCLYRICSANSWER|PM|AU|VM)$"
    )
    & ~BANNED_USERS
)
@languageCB
async def without_Admin_rights(client, CallbackQuery, _):
    ...
    if command == "AUTOPLAYANSWER":
        try:
            return await CallbackQuery.answer("ᴀᴜᴛᴏᴘʟᴀʏ ᴍᴏᴅᴇ ᴇɴᴀʙʟᴇs ᴛʜᴇ ʙᴏᴛ ᴛᴏ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴘʟᴀʏ sᴏɴɢs ᴡʜᴇɴ ᴛʜᴇ ǫᴜᴇᴜᴇ ɪs ᴇᴍᴘᴛʏ.", show_alert=True)
        except:
            return
    if command == "SYNCLYRICSANSWER":
        try:
            return await CallbackQuery.answer("ᴛᴜʀɴɪɴɢ ᴏɴ sʏɴᴄ ʟʏʀɪᴄs ғᴏʀ ᴛʜɪs ᴄʜᴀᴛ ᴡɪʟʟ ʀᴇᴘʟᴀʏ ᴛʜᴇ ɪɴʟɪɴᴇ ᴘʀᴏɢʀᴇss ʙᴀʀ !!", show_alert=True)
        except:
            return
    ...
    if command == "PM":
        ...
        if playty == "Everyone":
            ...
            Playtype = True
        autop = await is_autoplay(CallbackQuery.message.chat.id)
        buttons = playmode_users_markup(_, Direct, Group, Playtype, autop)
    if command == "AU":
        try:
...
@app.on_callback_query(
    filters.regex(pattern=r"^(MODECHANGE|CHANNELMODECHANGE|PLAYTYPECHANGE|AUTOPLAYCHANGE|SYNCLYRICSCHANGE)$")
    & ~BANNED_USERS
)
@ActualAdminCB
async def playmode_ans(client, CallbackQuery, _):
    ...
    if command == "AUTOPLAYCHANGE":
        ...
        try:
            ...
            pass
        if await is_autoplay(CallbackQuery.message.chat.id):
        ...
        if playty == "Everyone":
            ...
            Playtype = False
        buttons = playmode_users_markup(_, Direct, Group, Playtype, autop)
    if command == "SYNCLYRICSCHANGE":
        if CallbackQuery.from_user.id not in SUDOERS and CallbackQuery.from_user.id != OWNER_ID:
            return await CallbackQuery.answer("🚫 ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴏʀ ᴛʜᴇ ᴏᴡɴᴇʀ ᴄᴀɴ ᴍᴀɴᴀɢᴇ sʏɴᴄ ʟʏʀɪᴄs.", show_alert=True)
        try:
            await CallbackQuery.answer(_["set_cb_3"], show_alert=True)
        except:
            pass
        if await is_sync_lyrics(CallbackQuery.message.chat.id):
            await sync_lyrics_off(CallbackQuery.message.chat.id)
            sync_l = False
        else:
            await sync_lyrics_on(CallbackQuery.message.chat.id)
            sync_l = True
        playmode = await get_playmode(CallbackQuery.message.chat.id)
        if playmode == "Direct":
            Direct = True
        ...
        if playty == "Everyone":
            ...
            Playtype = False
        autop = await is_autoplay(CallbackQuery.message.chat.id)
        buttons = playmode_users_markup(_, Direct, Group, Playtype, autop, sync_l)
    try:
        return await CallbackQuery.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(buttons)
        )
